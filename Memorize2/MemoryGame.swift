...
struct MemoryGame<CardContent>  where CardContent: Equatable {
    ...
    // not used / replaced by cards.firstIndex() call above
    ...
    struct Card: Equatable, Identifiable, CustomDebugStringConvertible {
        ...
        private mutating func startUsingBonusTime() {
            if isFaceUp && !isMatched && bonusPercentReamining > 0, lastFaceUpDate == nil {
                lastFaceUpDate = Date()
            }
        }
        ...
        var bonus: Int {
            Int(bonusTimeLimit * bonusPercentReamining)
        }
        
        // percentage of the bonus time remaining
        var bonusPercentReamining: Double {
            bonusTimeLimit > 0 ? max(0, bonusTimeLimit - faceUpTime)/bonusTimeLimit : 0
        }
        
        // how long this card has even been face up and unmatched during its lifetime
